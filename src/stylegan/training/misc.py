...
def open_file_or_url(file_or_url):
    if gan_streamer.dnnlib.util.is_url(file_or_url):
        return gan_streamer.dnnlib.util.open_url(file_or_url, cache_dir=config.cache_dir)
...
def locate_run_dir(run_id_or_run_dir):
    if isinstance(run_id_or_run_dir, str):
        ...
        converted = gan_streamer.dnnlib.submission.submit.convert_path(run_id_or_run_dir)
...
def list_network_pkls(run_id_or_run_dir, include_final=True):
    ...
    if len(pkls) >= 1 and os.path.basename(pkls[0]) == 'albums.pkl':
        if include_final:
            pkls.append(pkls[0])
        del pkls[0]
    return pkls
    ...
    for candidate in [snapshot_or_network_pkl, run_id_or_run_dir_or_network_pkl]:
        if isinstance(candidate, str):
            if os.path.isfile(candidate):
                return candidate
            converted = gan_streamer.dnnlib.submission.submit.convert_path(candidate)
