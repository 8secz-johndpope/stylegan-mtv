...
class MetricBase:
    ...
    def run(self, network_pkl, run_dir=None, dataset_args=None, mirror_augment=None, num_gpus=1, tf_config=None, log_results=True):
        ...
        if log_results:
            ...
            if run_dir is not None:
                ...
                with gan_streamer.dnnlib.util.Logger(log, 'a'):
                    print(result_str)
            else:
                print(result_str)
    ...
    def get_result_str(self):
        ...
        result_str += ' time %-12s' % gan_streamer.dnnlib.util.format_time(self._eval_time)
    ...
    def update_autosummaries(self):
        for res in self._results:
            gan_streamer.dnnlib.tflib.autosummary.autosummary('Metrics/' + self.name + res.suffix, res.value)
...
class MetricGroup:
    def __init__(self, metric_kwarg_list):
        self.metrics = [gan_streamer.dnnlib.util.call_func_by_name(**kwargs) for kwargs in metric_kwarg_list]
